...
- `ticket_types` — id, event_id, name, price (pence), **capacity** (optional per-type sell cap; placeholder = event capacity when uncapped — see shared-pool note), tickets_sold, **units**, active, sort_order, created_at
...
**Shared-pool capacity (`units`) + optional per-type cap (`capacity`).** `events.capacity` is a single shared pool measured in **people**, and `events.tickets_sold` counts people consumed. Each ticket type has a `units` integer (people per ticket): **Single = 1, Duo = 2, custom default 1**. The admin form ships hardcoded **+ Single Ticket / + Duo Ticket / + Early Bird** preset buttons (plus **+ Custom**); the ticket-type row's third box is **People** (the `units` value) and the fourth box is **Cap** (see below).

A purchase of `quantity` tickets of a type consumes `quantity * units` people: `create-payment-intent` validates `quantity * units <= (event.capacity - event.tickets_sold)`, and `stripe-webhook` increments `events.tickets_sold` by `quantity * units` (passing `units` in PaymentIntent metadata).

**`ticket_types.capacity` is the type's optional sell cap** — the max number of *tickets* of that type that can sell (e.g. an **Early Bird** limited to the first 20). It is **a real binding limit again**: `create-payment-intent` rejects a purchase when `quantity > (type.capacity - type.tickets_sold)`, and `ticket_types.tickets_sold` (incremented by ticket *count* in `stripe-webhook`) is what it's measured against. **Uncapped types** (Single/Duo/Custom with the Cap box left blank) store `capacity = events.capacity` as a placeholder, so the cap term never binds before the shared people pool. The admin's **Cap** input (`.tt-cap`) writes this: blank → event capacity, a number → that cap. On edit the cap box shows a value only when `t.capacity < ev.capacity` (i.e. a real cap). Front-end availability (`events.html`) uses `typeAvail(ev, t) = min(floor(peopleRemaining(ev) / typeUnits(t)), t.capacity - t.tickets_sold)`: a type is sold out when `typeAvail <= 0`, and the per-type "X left" / max quantity in the checkout modal is `typeAvail`. (This deliberately reintroduces a per-type check for the Early Bird cap; the *shared people pool* still governs overall capacity.)
...
- **Lazy-loaded card images.** Both `events.html`'s `renderEventCard(ev, idx)` and `index.html`'s `renderEventCard(ev, idx)` add `loading="lazy"` to every event image except the first card (`idx === 0`), which loads eagerly so the first visible card isn't delayed. This is purely a fetch-timing hint — no visual or data effect — added to cut down Supabase Storage "Cached Egress" (see Images section below).
...
**Compress event images before uploading — `admin-upload-image.js` does no server-side resizing or compression.** It uploads whatever bytes the admin sends as-is (Storage Image Transformations is unavailable on the Supabase Free plan). Full-resolution phone/camera photos uploaded as PNG/JPEG directly caused the project's Supabase "Cached Egress" quota (5GB/mo) to be exceeded (198% used, ~9.9GB) from just a handful of event images on `index.html`/`events.html` being fetched by normal traffic — PNG especially, since it's lossless and compresses photographic detail poorly (a 3.8MB photo re-saved as truly lossless PNG only shrank to 3.63MB). Before uploading a new event image via the admin panel, re-encode it as WebP at quality ~90 (`sharp('in.png').webp({ quality: 90 }).toFile('out.webp')`, `sharp` is already a devDependency) — this cuts file size by roughly 85-90% with no visible quality/color loss, and `admin-upload-image.js` already accepts `webp` in its extension whitelist. Old images aren't auto-deleted from the bucket when an event's image is replaced; they just sit as orphaned Storage Size (a separate, much smaller quota than Cached Egress).
