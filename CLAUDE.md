...
- **Single start time only.** Cards/modals show the event's start time (the DB has no end-time column). The Summer Series events put the full "10:00 AM – 3:00 PM" style range as a `When:` line inside the `[[DETAILS]]` body. To show ranges on the cards themselves, an `end_time` column + admin field would be needed.
