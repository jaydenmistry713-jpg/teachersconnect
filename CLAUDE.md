...
- **`[[TIME:...]]` time-range marker.** Because the DB has no end-time column, the displayed time is overridden by an optional `[[TIME:10:00 AM – 3:00 PM]]` marker placed anywhere in the `description` (`splitDescription()` extracts it and strips it from the visible text). When present it replaces the start-time on both the card clock line and the details-modal header; when absent the formatted start time (`date.time`, UTC) is used. **Keep the `[[TIME:...]]` marker when editing a Summer Series event** or the card will fall back to showing only the start time. The three live events store their range in this marker (When/Where lines were removed from the `[[DETAILS]]` body since the modal header already shows date, range, and location).
...
- **Lower-page background.** The events section (`.events-section`) and community section (`.lower-section`) use an off-white base (`#FCFCFA`) with a faint teal radial-dot texture and soft green/orange corner glows (`.events-section::before`/`::after`), to differentiate from the plain cream hero while staying on-theme.
